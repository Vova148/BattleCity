...
StartScreen::StartScreen(const std::vector<std::string>& startScreenDescription, Game* pGame)
    ...
    , m_tanksSpriteAnimator(m_tankSprite.first)
{
...
void StartScreen::render()const
{
    ...
    m_menuSprite.first->render(m_menuSprite.second, glm::vec2(MENU_WIDTH, MENU_HEIGHT), 0);
    m_tankSprite.first->render(glm::vec2(m_tankSprite.second.x, m_tankSprite.second.y - m_currentMenuSelection*2*BLOCK_SIZE), glm::vec2(TANK_SIZE), 0.f, 0.f, m_tanksSpriteAnimator.getCurrentFrame());
}

void StartScreen::update(const double delta)
{
    m_tanksSpriteAnimator.update(delta);
}
